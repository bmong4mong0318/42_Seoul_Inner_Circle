...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/08 13:34:18 by dayun             #+#    #+#             */
/*   Updated: 2023/05/08 13:35:12 by dayun            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PhoneBook::ValidateInputIndex(std::string inputIndex)
{
  ...
  if (isdigit(inputIndex[0]) == 0)
    throw "Invalid input - It's not integer\n";
  if (!(std::atoi(inputIndex.c_str()) <= 8 && std::atoi(inputIndex.c_str()) >= 1))
    throw "Invalid input - Out of range\n";
  if (this->idx < 8 && std::atoi(inputIndex.c_str()) >= this->idx + 1)
    throw "Invalid input - Out of range\n";
}
